...
The GHCM as implemented in the \code{ghcm_test} function assumes that the all the residuals of the $X$ on $Z$ regression are evaluated on the same grid and that residuals of the $Y$ on $Z$ regression are evaluated on the same grid. However, these two grids can be distinct. 

In the \code{ghcm_test_splines} function this assumption is relaxed and the functions can be observed on arbitrary, irregular grids as long as each curve has 4 observations or more. 
...
test <- ghcm_test_spline(resid(m_X), resid(m_1), X_limits=c(0, 1))
print(test)
```
We get a $p$-value of $0.883$, which is very similar to the $p$-value of the regularly observed data of $0.865$.
...
test <- ghcm_test_spline(resid(m_X), resid(m_W), X_limits=c(0, 1), Y_limits=c(0, 1))
print(test)
```
We get a $p$-value of $0.592$ which is again similar to the $p$-value of the regularly observed data of $0.6304$.
