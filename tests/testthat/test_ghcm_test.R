...
resid_X_on_Z_functional <- matrix(rnorm(length(grid_X) * N), ncol=length(grid_X))
resid_Y_on_Z_scalar <- rnorm(N)
resid_Y_on_Z_functional <- matrix(rnorm(length(grid_Y) * N), ncol=length(grid_Y))
...
test_that("ghcm_runs_with_functional_and_scalar_inputs", {
  tst <- ghcm_test(resid_X_on_Z_functional, resid_Y_on_Z_scalar, X_grid=grid_X)
  expect_is(tst, "ghcm")
})
...
test_that("ghcm_fails_when_giving_bad_fpca_string", {
  expect_error({
    ghcm_test(resid_X_on_Z_functional, resid_Y_on_Z_functional,
              X_grid=grid_X, Y_grid=grid_Y, fpca_method = "not_an_fpca_method")
  })
})
